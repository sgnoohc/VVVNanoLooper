...
void Begin_4LepMET()
{
    ...
    // Select 4 leptons
    ana.cutflow.addCutToLastActiveCut("4LepMET_Preselection",
            [&]()
            {
                return (ana.tx.getBranchLazy<vector<int>>("Common_muon_idxs").size() + ana.tx.getBranchLazy<vector<int>>("Common_electron_idxs").size() == 4);
            },
            [&]()
            {
                return ana.tx.getBranch<float>("4LepMET_floatVar1");
            } );
