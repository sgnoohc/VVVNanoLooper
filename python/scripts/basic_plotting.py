...
def drawTwoPads(top_hists_i, bottom_hists_i, savename="", logTop=False, logBottom=False):

    bottom_hists = []
    top_hists = []
    for h in bottom_hists_i:
        tmp = h.Clone()
        bottom_hists.append(tmp)
    
    for h in top_hists_i:
        tmp = h.Clone()
        top_hists.append(tmp)

    c.Clear()
    leg = ROOT.TLegend(0.50,0.75,0.70,0.92)
    leg.SetFillStyle(0)
    ...
    pad1.SetBottomMargin(0.05) # Upper and lower plot are close
    pad1.Draw()             # Draw the upper pad: pad1
    pad1.cd()               # pad1 becomes the current pad

    top_hists[0].Draw("hist")
    for h in top_hists:
        h.SetLineWidth(2)
        #h.SetLineColor(more_colors[i])
    ...
    top_hists[0].Draw("hist same")
    leg.Draw()

    # lower plot will be in pad
    c.cd()          # Go back to the main canvas before defining pad2
    pad2 = ROOT.TPad("pad2", "pad3", 0, 0.05, 1, 0.3)
    pad2.SetTopMargin(0)
    ...
    for bottom_hist in bottom_hists:
        bottom_hist.Draw("ep same")
    ...
    bottom_hists[0].GetYaxis().SetNdivisions(505)
    bottom_hists[0].GetYaxis().SetTitleSize(15)
    bottom_hists[0].GetYaxis().SetTitleFont(43)
    bottom_hists[0].GetYaxis().SetTitleOffset(1.55)
    bottom_hists[0].GetYaxis().SetLabelFont(43) # Absolute font size in pixel (precision 3)
    bottom_hists[0].GetYaxis().SetLabelSize(15)
    bottom_hists[0].GetYaxis().SetTitle("CR/SR")
    bottom_hists[0].SetMinimum(0)
    bottom_hists[0].SetMaximum(getMaximum(bottom_hists))
   

    # X axis ratio plot settings
    bottom_hists[0].GetXaxis().SetTitleSize(0.5)
    bottom_hists[0].GetXaxis().SetTitleFont(42)
    #bottom_hists[0].GetXaxis().SetTitleOffset(4.)
    bottom_hists[0].GetXaxis().SetLabelFont(43) # Absolute font size in pixel (precision 3)
    bottom_hists[0].GetXaxis().SetLabelSize(15)
    bottom_hists[0].GetXaxis().SetLabelOffset(.02)
    
    l = ROOT.TLine(bottom_hists[0].GetXaxis().GetXmin(),1.0,bottom_hists[0].GetXaxis().GetXmax(),1.0)
...
def getMaximum(hists, isLog=False):
    ...
    for i in xrange(len(hists)):
        print hists[i].GetTitle(), hists[i].GetMaximum()
...
def get_xsec_lumi_scaled_histogram(tfile, name, year, isEFT, x2=False):
