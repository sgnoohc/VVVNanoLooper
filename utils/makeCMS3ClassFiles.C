...
void makeHeaderFile(TFile *f, const string& treeName, bool paranoid, const string& Classname, const string& nameSpace, const string& objName)
{
  ...
  for (Int_t i = 0; i < aliasarray->GetSize(); ++i) {
    ...
    if (classname.Contains("vector")) {
      if(classname.Contains("edm::Wrapper<") ) {
        ...
        headerf << "/*" << setw(40) << aliasname << "*/" << "  " << classname << " " << aliasname << "_;" << endl;
      }
      ...
      if(classname != "" ) { //LorentzVector
        if(classname.Contains("edm::Wrapper<") ) {
          ...
          classname.ReplaceAll("edm::Wrapper<","");
          headerf << "/*" << setw(40) << aliasname << "*/" << "  " << setw(8) << left << classname << " " << aliasname << "_;" << endl;
        }
        //else if (classname.Contains("TString")) {
        //    headerf << "  " << classname << " " << aliasname << "_;" << endl;
        //}
        ...
        if(title.EndsWith("/i"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  unsigned int " << aliasname << "_;" << endl;
        if(title.EndsWith("/l"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  unsigned long long " << aliasname << "_;" << endl;
        if(title.EndsWith("/F"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  float    " << aliasname << "_;" << endl;
        if(title.EndsWith("/I"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  int      " << aliasname << "_;" << endl;
        if(title.EndsWith("/O"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  bool     " << aliasname << "_;" << endl;
        if(title.EndsWith("/D"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  double   " << aliasname << "_;" << endl;
      }
    }
  ...
  for (Int_t i = 0; i< aliasarray->GetSize(); i++) {
    ...
    if (classname.Contains("vector")) {
      ...
      headerf << "/*" << setw(40) << aliasname << "*/" << "  const " << classname << " &" << aliasname << "();" << endl;
    } else {
      if(classname.Contains("edm::Wrapper<")) {
        classname = classname(0,classname.Length()-1);
        classname.ReplaceAll("edm::Wrapper<","");
      ...
      if(classname != "" ) {
        ...
        if(title.EndsWith("/i"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  const unsigned int &" << aliasname << "();" << endl;
        if(title.EndsWith("/l"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  const unsigned long long &" << aliasname << "();" << endl;
        if(title.EndsWith("/F"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  const float &" << aliasname << "();" << endl;
        if(title.EndsWith("/I"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  const int &" << aliasname << "();" << endl;
        if(title.EndsWith("/O"))
          headerf << "/*" << setw(40) << aliasname << "*/" << "  const bool &" << aliasname << "();" << endl;
        if(title.EndsWith("/D"))
...
void makeCCFile(TFile *f, const string& Classname, const string& nameSpace, const string& branchNamesFile, const string& treeName, const string& objName, bool paranoid) {
    ...
    if (!classname.Contains("vector<vector")) {
      if (classname.Contains("Lorentz") || classname.Contains("PositionVector") || classname.Contains("TBits")) {
        ...
        if (have_aliases) {
          ...
          implf << "/*" << setw(40) << aliasname << "*/" << "  " << Form("%s_branch",aliasname.Data()) << " = tree->GetBranch(\"" << aliasname << "\");" << endl;
          //implf << "    " << Form("%s_branch",aliasname.Data()) << "->SetAddress(&" << aliasname << "_);" << endl << "  }" << endl;
          implf << "/*" << setw(40) << aliasname << "*/" << Form("  if (%s) %s->SetAddress(&%s_);", branch_ptr.Data(), branch_ptr.Data(), aliasname.Data()) << endl;
        }
        implf << "//---------------------------------------------------------------------------------" << endl;
  ...
  for (Int_t i = 0; i< aliasarray->GetSize(); i++) {
    ...
    implf << "/*" << setw(40) << aliasname << "*/" << "  " << "if (not " << Form("%s_isLoaded) {",aliasname.Data()) << endl;
    implf << "/*" << setw(40) << aliasname << "*/" << "    " << "if (" << Form("%s_branch",aliasname.Data()) << " != 0) {" << endl;
    implf << "/*" << setw(40) << aliasname << "*/" << "      " << Form("%s_branch",aliasname.Data()) << "->GetEntry(index);" << endl;
    if (paranoid) {
      implf << "/*" << setw(40) << aliasname << "*/" << "      #ifdef PARANOIA" << endl;
      if (classname == "vector<vector<float> >") {
        if (isSkimmedNtuple) {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "for (vector<vector<float> >::const_iterator i = "
                << aliasname << "_->begin(); i != "<< aliasname << "_->end(); ++i) {" << endl;
        } else {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "for (vector<vector<float> >::const_iterator i = "
                << aliasname << "_.begin(); i != "<< aliasname << "_.end(); ++i) {" << endl;
        }
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "for (vector<float>::const_iterator j = i->begin(); "
          "j != i->end(); ++j) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          " << "if (not isfinite(*j)) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "            " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
              << " contains a bad float: %f\\n\", *j);" << endl << "            " << "exit(1);"
              << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          }\n        }\n      }" << endl;
      } else if (classname == "vector<float>") {
        if (isSkimmedNtuple) {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "for (vector<float>::const_iterator i = "
                << aliasname << "_->begin(); i != "<< aliasname << "_->end(); ++i) {" << endl;
        } else {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "for (vector<float>::const_iterator i = "
                << aliasname << "_.begin(); i != "<< aliasname << "_.end(); ++i) {" << endl;
        }
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "if (not isfinite(*i)) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
              << " contains a bad float: %f\\n\", *i);" << endl << "          " << "exit(1);"
              << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "        }\n      }" << endl;
      } else if (classname == "float") {
        implf << "/*" << setw(40) << aliasname << "*/" << "      " << "if (not isfinite(" << aliasname << "_)) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
              << " contains a bad float: %f\\n\", " << aliasname << "_);" << endl
              << "        " << "exit(1);"
              << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "      }" << endl;
      } else if (classname.BeginsWith("vector<vector<ROOT::Math::LorentzVector")) {
        if (isSkimmedNtuple) {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "for (" << classname.Data() <<"::const_iterator i = "
                << aliasname << "_->begin(); i != "<< aliasname << "_->end(); ++i) {" << endl;
        } else {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "for (" << classname.Data() <<"::const_iterator i = "
                << aliasname << "_.begin(); i != "<< aliasname << "_.end(); ++i) {" << endl;
        ...
        str[str.length() - 2] = 0;
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "for (" << str.c_str() << "::const_iterator j = i->begin(); "
          "j != i->end(); ++j) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          " << "int e;" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          " << "frexp(j->pt(), &e);" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          " << "if (not isfinite(j->pt()) || e > 30) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "            " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
              << " contains a bad float: %f\\n\", j->pt());" << endl << "            " << "exit(1);"
              << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          }\n        }\n      }" << endl;
        ...
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "int e;" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "frexp(i->pt(), &e);" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "        " << "if (not isfinite(i->pt()) || e > 30) {" << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "          " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
              << " contains a bad float: %f\\n\", i->pt());" << endl << "          " << "exit(1);"
              << endl;
        implf << "/*" << setw(40) << aliasname << "*/" << "        }\n      }" << endl;
        ...
        implf << "/*" << setw(40) << aliasname << "*/" << "      " << "int e;" << endl;
        if (isSkimmedNtuple) {
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "frexp(" << aliasname << "_->pt(), &e);" << endl;
          implf << "/*" << setw(40) << aliasname << "*/" << "      " << "if (not isfinite(" << aliasname << "_->pt()) || e > 30) {" << endl;
          implf << "/*" << setw(40) << aliasname << "*/" << "        " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
                << " contains a bad float: %f\\n\", " << aliasname << "_->pt());" << endl
                << "        " << "exit(1);"
                << endl;
      ...
      implf << "/*" << setw(40) << aliasname << "*/" << "      #endif // #ifdef PARANOIA" << endl;
    }
    implf << "/*" << setw(40) << aliasname << "*/" << "    " << "} else {" << endl;
    implf << "/*" << setw(40) << aliasname << "*/" << "      " << "printf(\"branch " << Form("%s_branch",aliasname.Data())
          << " does not exist!\\n\");" << endl;
    implf << "/*" << setw(40) << aliasname << "*/" << "      " << "exit(1);" << endl << "/*" << setw(40) << aliasname << "*/" << "    }" << endl;
    implf << "/*" << setw(40) << aliasname << "*/" << "    " << Form("%s_isLoaded",aliasname.Data()) << " = true;" << endl;
    implf << "/*" << setw(40) << aliasname << "*/" << "  " << "}" << endl;
    if (isSkimmedNtuple) {
